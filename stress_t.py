...
def generate_stress_test_data(raw_data_directory, files_count, data_assets_count, days_count):
    ...
    for _ in range(data_assets_count):
        ...
        data_asset = build_data_asset(timestamp=timestamp)
    ...
    while True:
        time.sleep(3)
        msg_count = number_of_messages_in_sqs_queue()
        if msg_count == 0:
            break
...
def number_of_messages_in_sqs_queue():
    sqs_client = boto3.client("sqs")
    response = sqs_client.get_queue_attributes(
        QueueUrl=UPLOADER_SQS_QUEUE_URL, AttributeNames=["ApproximateNumberOfMessages"]
    )
...
def main():
    ...
    parser.add_argument("--files", type=int, required=True, help="Number of files to generate in total")
    ...
    if args.raw_data_directory:
        generate_stress_test_data(args.raw_data_directory, args.files, args.assets_per_file, args.days_count)
    else:
